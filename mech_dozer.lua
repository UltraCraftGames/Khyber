...
-- artillery with gravity higher than this
-- has reduced artillery flight time.
local DEF_GRAVITY = Values["gravity"]	-- (3)

-- projectiles with higher velocity than this
-- has reduces projectile flight time.
local DEF_VEL_X = Values["x_velocity"]	-- (0.7)

-- artillery with velocity higher than this
-- has increses artillery flight time.
local DEF_VEL_Y = Values["y_velocity"]	-- (18)


-- returns the duration of a projectile, given distance and velocity.
local function GetProjectileDuration(distance, vel_x)
	return distance * 0.09 * DEF_VEL_X / vel_x
end

-- returns the duration of an artillery shot, given gravity and velocity.
-- '0.8' is an approximate value that may need
-- modification depending on vel_y or gravity used.
local function GetArtilleryDuration(gravity, vel_y)
	return 0.8 * vel_y * DEF_GRAVITY / (DEF_VEL_Y * gravity)
end
...
-- this function will move all pawns away from a tile.
-- meant to be used in conjunction with RewindTile()
local function ClearTile(effect, p1)
	effect:AddScript([[
		lmn_dozer_displaced = {};
		...
		while pawn do
			...
			table.insert(lmn_dozer_displaced, pawn:GetId());
			pawn = Board:GetPawn(point);
		end;
	]])
end

-- this function will move all pawns away from a tile, except the one we are searching for.
-- meant to be used in conjunction with RewindTile()
local function FilterTile(effect, p1, id)
	effect:AddScript([[
		lmn_dozer_displaced = {};
		local point = Point(]].. p1.x ..",".. p1.y ..[[);
		local pawn = Board:GetPawn(point);
...
-- this function will move all displaced pawns back to the tile.
-- meant to be used in conjunction with FilterTile() or ClearTile()
local function RewindTile(effect, p1)
	effect:AddScript([[
		if type(lmn_dozer_displaced) == "table" then
			for _, id in ipairs(lmn_dozer_displaced) do
...
-- this function will charge a pawn, but it won't preview it.
local function HiddenCharge(effect, pawn, p1, p2)
	FilterTile(effect, p1, pawn:GetId())
	-- charge tile via script so the previewer won't know about it.
	effect:AddScript([[
		local effect = SkillEffect();
		local p1 = Point(]].. p1.x ..",".. p1.y ..[[);
		local p2 = Point(]].. p2.x ..",".. p2.y ..[[);
		effect:AddCharge(Board:GetPath(p1, p2, PATH_FLYER), NO_DELAY);
		Board:AddEffect(effect);
	]])
	RewindTile(effect, p1)
end

-- this function will preview a charge, but won't actually move anything.
local function FakePreviewCharge(effect, p1, p2)
	ClearTile(effect, p1)
	effect:AddCharge(Board:GetPath(p1, p2, PATH_FLYER), NO_DELAY)
	RewindTile(effect, p1)
end

-- this function will preview damage, but won't apply it when using the skill.
local function FakePreviewDamage(effect, p1, spaceDamage)
	ClearTile(effect, p1)
...
-- this function will change the velocity of 
-- AddProjectile and AddCharge for every skill.
-- in order to only affect a single projectile or charge,
-- we need to reset the velocity afterwards with ResetVelocity.

-- use NO_DELAY for the projectile or charge,
-- so the value is reset on the same frame it was changed.
function lmn_DozerAtk:SetVelocity(effect)
	if not orig_vel_x then
		orig_vel_x = Values["x_velocity"]
		effect:AddScript("Values['x_velocity'] = ".. self.VelX)
	end
end

-- reset charge velocity.
function lmn_DozerAtk:ResetVelocity(effect)
	if orig_vel_x then
		effect:AddScript("Values['x_velocity'] = ".. orig_vel_x)
		orig_vel_x = nil
	end
end
...
function lmn_DozerAtk:GetTargetArea(point)
	...
	for dir = DIR_START, DIR_END do
		...
		if mech.isTipImage then
			mech.isImageMarkTip[dir] = {}
			isImageMark = mech.isImageMarkTip[dir]
		else
			mech.isImageMark[dir] = {}
			isImageMark = mech.isImageMark[dir]
		end
	...
	for k = 1, distance do
		...
		if charges[k] then													-- charge pushable pawns at their correct timings.
			...
			for _, v in ipairs(charges[k]) do
				...
				-- script to sort a tile to filter out the correct pawn to charge.
				FilterTile(ret, start, v.id)
				
				self:SetVelocity(ret)
				ret:AddCharge(Board:GetPath(start, stop, PATH_FLYER), NO_DELAY)
				self:ResetVelocity(ret)
				
				-- script to move all displaced pawns back to the tile.
				RewindTile(ret, start)
				
				if start ~= v.initLoc then
					FakePreviewCharge(ret, v.initLoc, stop)
				end
			end
		end
		...
		if k <= data.distanceMoved then
			ret:AddDelay(0.08 * DEF_VEL_X / self.VelX)
		end
	end
	
	self:SetVelocity(ret)
	FakePreviewCharge(ret, p1, moveLoc)
	self:ResetVelocity(ret)
...
function lmn_DozerAtk_Tip:GetTargetArea(p, bool)
	...
	mech.isTipImage = true
	local ret = lmn_DozerAtk.GetTargetArea(self, p, bool)
	mech.isTipImage = nil
	return ret
end

function lmn_DozerAtk_Tip:GetSkillEffect(p1, p2, bool)
	mech.isTipImage = true
	local ret = lmn_DozerAtk.GetSkillEffect(self, p1, p2, bool)
	mech.isTipImage = nil
	return ret
end
...
function mech:load(options, modApiExt)
